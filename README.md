...
## ✨ 2.1.36 看点
- 📦 **外部 Compose 项目发现与接管**：自动发现宿主机上用 compose 创建的项目（SSH / 群晖套件 / dockge 等部署的都算），可读取原 docker-compose.yaml（不可读时按容器配置反向生成），一键导入为托管项目并平滑接管现有容器，不停机
- ⬆️ **项目更新重建**：容器项目支持「更新重建」——拉取最新镜像后强制重建，确认弹窗列出受影响容器并标注哪些检出新镜像，卷数据不受影响
- 🆕 「新建」页新增「外部项目」来源：与可视化表单、命令行转换、Compose 编辑、容器生成并列，导入前可编辑核对
- 🔍 修复 2.1.25 起容器更新检测漏检（恢复加速器回退与 HEAD 探测），检测候选源纳入用户配置的加速源
- 🔓 修复 `config.json` 在宿主机无法查看编辑的权限问题（默认 0666，可用 `DOCKERCOPILOT_CONFIG_FILE_MODE`/`PUID`/`PGID` 调整）
